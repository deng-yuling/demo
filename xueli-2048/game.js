(function () {
  ...
  const { addGameRecord, getRankedHistory, formatPlayedAt } = window.Game2048History;

  const BEST_KEY = "xueli2048_best";
  const LEGACY_BEST_KEY = "moe2048_best";
  ...
  function renderHistoryRanking() {
    ...
    historyCount.textContent = ranked.length;
    ...
    ranked.forEach((item) => {
      ...
      if (item.rank <= 3) li.classList.add(`top-${item.rank}`);

      li.innerHTML = `
        <span class="history-rank">${item.rank}</span>
        <span class="history-game">第 ${item.gameNo} 局</span>
        <span class="history-score">${item.score}</span>
        <span class="history-time">${formatPlayedAt(item.playedAt)}</span>
      `;
  ...
  function renderTiles(animHints) {
    ...
    const hints = animHints || { newCells: [] };
    ...
    for (let r = 0; r < SIZE; r++) {
      for (let c = 0; c < SIZE; c++) {
        ...
        if (hints.newCells.some((x) => x.r === r && x.c === c)) {
  ...
  function newGame() {
    if (!scoreSaved && score > 0 && !canMove(grid)) {
      saveScoreToHistory();
    }
    grid = emptyGrid();
    score = 0;
